...
堆排序：所使用的最大堆 / 最小堆结构本质上是一棵完全二叉树。而完全二叉树适合采用顺序存储结构（数组）。因为数组存储的完全二叉树可以很方便的通过下标序号来确定父亲节点和孩子节点，并且可以极大限度的节省存储空间。
...
4. 一次循环之后，将 `tail` 移动到 `node_j` 所在位置。相当于 `tail` 进行左移了一位。此时 `tail` 节点右侧为链表中最大的链节点。

...
> **图的网络流**：假设存在有向带权图 $G = (V, E)$，$V = {s, v_1, v_2, ..., t}$，其中有两个特殊顶点 $s$ 和 $t$，$s$ 叫做源点，$t$ 叫做汇点。边的方向表示允许的流向；边上的权值表示该边允许通过的最大流量 $cap (cap \ge 0)$，即边的容量；如果在该图中有一条边 $(u, v)$，则不存在反向的边 $(v, u)$。这样的有向带权图就叫做网络。
> 
